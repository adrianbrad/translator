...
func TestTranslator_GetTranslation_ErrorParsingLanguageTo(t *testing.T) {
	...
	assert.NotNil(t, err)
	assert.Equal(t, "Error while parsing language tag 2: language: tag is not well-formed", err.Error())
}
...
func TestTranslator_GetTranslation_ErrorParsingLanguageFrom(t *testing.T) {
	...
	assert.Empty(t, textTo)
	assert.NotNil(t, err)
	assert.Equal(t, "Error while parsing language tag 1: language: tag is not well-formed", err.Error())
}
