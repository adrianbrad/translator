...
run-cli-db:
	...
	DB_HOST=$(DB_HOST) DB_PORT=$(DB_PORT) DB_USER=$(DB_USER) DB_PASS=$(DB_PASS) ./cmd/translator-db/cli/translator-db-cli -race

run-web-db:
	cd cmd/translator-db/web; go build -v -o translator-db-web -mod vendor -race -tags='db web'
	DB_HOST=$(DB_HOST) DB_PORT=$(DB_PORT) DB_USER=$(DB_USER) DB_PASS=$(DB_PASS) ./cmd/translator-db/web/translator-db-web -p=$(PORT)

test-us-cli-mem:
	go test ./internal/test/userstory -run TestMemCLI -count=1 -v -race -tags='memory db cli web'

test-us-cli-db:
	DB_HOST=$(DB_HOST) DB_PORT=$(DB_PORT) DB_USER=$(DB_USER) DB_PASS=$(DB_PASS) go test ./internal/test/userstory -run TestDBCLI -count=1 -v -race -tags='memory db cli web'

test-us-web-mem:
	go test ./internal/test/userstory -run TestMemWeb -count=1 -v -race -tags='memory db cli web'

test-us-web-db:
	DB_HOST=$(DB_HOST) DB_PORT=$(DB_PORT) DB_USER=$(DB_USER) DB_PASS=$(DB_PASS) go test ./internal/test/userstory -run TestDBWeb -count=1 -v -race -tags='memory db cli web'